...
def func(datasets_and_tuples_and_return_data):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if return_data:
            ...
            # Write non-HRV
            write_region(dataset, path=non_hrv_tuple[0], times=non_hrv_tuple[1], x=non_hrv_tuple[2], y=non_hrv_tuple[3])
            write_region(hrv_dataset, path=hrv_tuple[0], times=hrv_tuple[1], x=hrv_tuple[2], y=hrv_tuple[3])
...
def write_region(data, path, x, y, times):
    ...
    data.to_zarr(path, region={"time": slice(i, i + 1), "y_geostationary": slice(0, 4176),
                                                    "x_geostationary": slice(0, len(data.coords['x_geostationary'].values)), "variable": slice(0, 1)})
    data.close()
...
def create_dummy_zarr(datasets, base_path):
    ...
    timestamps = []
    non_hrv_path = os.path.join(base_path, pd.Timestamp(eumetsat_filename_to_datetime(datasets[0]['id'])).round('5 min').strftime('%Y%m') + ".zarr")
    hrv_path = os.path.join(base_path, "hrv_" + pd.Timestamp(eumetsat_filename_to_datetime(datasets[0]['id'])).round('5 min').strftime('%Y%m') + ".zarr")
    for dataset in datasets:
        timestamps.append(pd.Timestamp(eumetsat_filename_to_datetime(dataset['id'])).round('5 min').to_datetime64())

    dummies = dask.array.zeros((len(datasets), 1392, 3712, 11), chunks=(1, 1392, 3712, 1), dtype="float32")
    ds = xr.Dataset(
    ...
    hrv_ds = xr.Dataset(
        ...
        coords=dict(
            x_geostationary=hrv.coords["x_geostationary"][:5568], # Some HRV files are 2 pixels smaller than rest, this just trims off 2 pixels, which should be okay?
    ...
    return (hrv_path, hrv_ds["time"].values, hrv_ds["x_geostationary"].values, hrv_ds["y_geostationary"].values), \
...
if __name__ == "__main__":
    ...
    for date in date_range[::-1]:
        ...
        for dataset in datasets:
            try:
                if os.path.exists(os.path.join("/mnt/storage_ssd_4tb/EUMETSAT_Zarr/",
                    ...
                    continue
                if os.path.exists(os.path.join("/home/jacob/mnt/storage_a/data/ocf/solar_pv_nowcasting/nowcasting_dataset_pipeline/satellite/EUMETSAT/SEVIRI_RSS/zarr/v6/",
                                               f"{pd.Timestamp(eumetsat_filename_to_datetime(dataset['id'])).round('5 min').strftime('%Y%m%d%H%M')}.zarr.zip")):
                    print(
                        f"Skipping Time {pd.Timestamp(eumetsat_filename_to_datetime(dataset['id'])).round('5 min').strftime('%Y%m%d%H%M')}")
                    continue
        ...
        hrv_tuple, non_hrv_tuple = create_dummy_zarr(datasets, "/mnt/storage_ssd_4tb/EUMETSAT_Zarr/")
        # Now map datasets to f
        pool = mp.Pool(processes=48)
        first = False
        for _ in tqdm(pool.imap_unordered(func, zip(datasets, repeat(hrv_tuple), repeat(non_hrv_tuple), repeat(False))), total=len(datasets)):
            continue
