...
stub["empty_app"] = modal.Conda().conda_install(["zarr", "s3fs", "fsspec", "xarray", "dask", "pandas", "numpy"]).pip_install(["satip"])

stub["app"] = modal.Conda().conda_install(["zarr", "s3fs", "fsspec", "xarray", "dask", "pandas", "numpy"]).pip_install(["satip"])
...
@stub.function(image=stub["app"], secret=modal.ref("eumetsat"), shared_volumes={CACHE_DIR: volume}, memory=8192, concurrency_limit=16)
...
def create_dummy_zarr(hrv_bytes, data_bytes, hrv_files, data_files):
    ...
    non_hrv_path = os.path.join(
        CACHE_DIR,
        data_files[0].split("/")[-1].split(".zarr")[0]
        + "_collated_4_6"
        + ".zarr",
    )
    hrv_path = os.path.join(
        CACHE_DIR,
        "hrv_"
        + hrv_files[0].split("/")[-1].split(".zarr")[0]
        + "_collated_4_6"
        + ".zarr",
    )
    for dataset in data_files:
        ...
        timestamps.append(
            pd.Timestamp(year=int(date[:4]), month=int(date[4:6]),
                         day=int(date[6:8]), hour=int(date[8:10]),
                         minute=int(date[10:12]))
            .to_datetime64()
        )

    dummies = dask.array.zeros(
        (len(data_files), 1392, 3712, 11), chunks=(1, int(1392/2), int(3712/4), 1), dtype="float32"
    )
    y_chord_dummies = dask.array.zeros(
        (len(data_files), 1392), chunks=(1, 1392), dtype="float32"
    )
    x_chord_dummies = dask.array.zeros(
        (len(data_files), 3712), chunks=(1, 3712), dtype="float32"
    )
    ds = xr.Dataset(
    ...
    hrv_dummies = dask.array.zeros(
        (len(hrv_files), 4176, 5568, 1), chunks=(1, int(4176/4), int(5568/4), 1), dtype="float32"
    )
    y_choord_dummies = dask.array.zeros(
        (len(hrv_files), 4176), chunks=(1, 4176), dtype="float32"
    )
    x_choord_dummies = dask.array.zeros(
        (len(hrv_files), 5568), chunks=(1, 5568), dtype="float32"
    )
    hrv_ds = xr.Dataset(
        data_vars=dict(
...
if __name__ == "__main__":
    ...
    data_files = sorted(list(glob.glob(os.path.join("/run/media/jacob/SSD2/modal/", "202106*.zarr.zip"))))
    print(len(data_files))
    hrv_data_files = sorted(list(glob.glob(os.path.join("/run/media/jacob/SSD2/modal/", "hrv_202106*.zarr.zip"))))
    print(len(data_files))
    ...
    with stub.run():
        ...
        hrv_tuple, non_hrv_tuple = create_dummy_zarr(tmp_hrv_bytes, tmp_data_bytes, hrv_data_files, data_files)

        # Now map datasets to f
        for _ in func.starmap(zip(load_bytes(data_files), load_bytes(hrv_data_files), repeat(hrv_tuple), repeat(non_hrv_tuple))):
            continue
