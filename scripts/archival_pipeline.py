...
def list_native_files():
    # Get native files in order
    native_files = list(glob.glob("/mnt/disks/sat/native_files/*/*.nat"))
...
if __name__ == "__main__":
    zarr_path = "/mnt/disks/sat/2024_hrv.zarr"
    if os.path.exists(zarr_path):
        hrv_zarr_times = xr.open_zarr(zarr_path).sortby("time").time.values
        last_zarr_time = hrv_zarr_times[-1]
    else:
        # Set dummy values for times already in zarr
        last_zarr_time = dt.datetime(1970, 1, 1)
        hrv_zarr_times = [last_zarr_time, last_zarr_time]
    ...
    for f in native_files:
        ...
        if len(hrv_datasets) == 12:
            ...
            if os.path.exists(zarr_path):
                mode = "a"
            write_to_zarr(
                xr.concat(hrv_datasets, dim="time"), zarr_path, mode, chunks={"time": 12,}
            )
            hrv_datasets = []
