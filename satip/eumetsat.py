...
__all__ = ['check_env_vars_have_loaded', 'request_access_token', 'query_data_products', 'format_dt_str',
           'identify_available_datasets', 'dataset_id_to_link', 'json_extract', 'extract_metadata', 'metadata_maps',
           'check_valid_request', 'DownloadManager', 'get_dir_size', 'get_filesize_megabytes',
           'eumetsat_filename_to_datetime', 'compress_downloaded_files']
...
class DownloadManager:
    ...
    def check_if_downloaded(self, filenames: List[str]):
        ...
        filenames = [re.match("([A-Z\d.]+-){6}", filename)[0][:-1] for filename in filenames]
...
def compress_downloaded_files(delete_original_files = True):
    ...
    for full_native_filename in full_native_filenames:
        ...
        print('Compressing %s', full_native_filename)
        ...
        print('Filesizes: Before compression = %.1f MB. After compression = %.1f MB.  Compressed file is %.1f x the size of the uncompressed file.',
                 native_filesize_mb, compressed_filesize_mb, compressed_filesize_mb / native_filesize_mb)
        ...
        print('Moving %s to %s', full_compressed_filename, new_dst_full_filename)

        if os.path.exists(new_dst_full_filename) and delete_original_files:
            log.debug('%s already exists.  Deleting old file', new_dst_full_filename)
            print('%s already exists.  Deleting old file', new_dst_full_filename)
