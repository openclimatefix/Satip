...
class DownloadManager:
        ...
        * Adding satip helper functions
        ...
        if bucket_name:
            ...
            self.bucket_filenames = [
                re.match("([A-Z\d.]+-){6}", filename)[0][:-1] for filename in filenames
            ]
    ...
    def check_if_downloaded(self, filenames: List[str]):
        ...
        if self.bucket_name:
            self.logger.info(
                f"{len(filenames)} files queried, {len(in_bucket)} found in bucket, {len(local)} found in {self.data_dir}, {len(download)} to download."
            )
        else:
            self.logger.info(
                f"{len(filenames)} files queried, {len(local)} found in {self.data_dir}, {len(download)} to download."
            )
    ...
    get_df_metadata = lambda self: pd.DataFrame(self.metadata_table.all()).set_index(
        "id"
    )
...
# Cell
def compress_downloaded_files(
    data_dir, sorted_dir, NATIVE_FILESIZE_MB=102.210123, log=None
):
    ...
    for full_native_filename in full_native_filenames:
        ...
        if log:
            log.debug(
                "Filesizes: Before compression = %.1f MB. After compression = %.1f MB.  Compressed file is %.1f x the size of the uncompressed file.",
                native_filesize_mb,
                compressed_filesize_mb,
                compressed_filesize_mb / native_filesize_mb,
            )
        ...
        new_dst_full_filename = os.path.join(
            new_dst_path, base_native_filename + EXTENSION
        )
        if log:
            log.debug(
                "Moving %s to %s", full_compressed_filename, new_dst_full_filename
            )

        if os.path.exists(new_dst_full_filename):
            if log:
                log.debug(
                    "%s already exists.  Deleting old file", new_dst_full_filename
                )
