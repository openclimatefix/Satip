...
__all__ = [
    ...
    "load_scene",
    "construct_area_def",
    "construct_TM_area_def",
    "reproj_with_manual_grid",
    "reproj_to_xarray",
    "full_scene_pyresample",
    "full_scene_pyinterp",
    "Reprojector",
]
...
def calculate_x_offset(native_fp):
    ...
    lower_east_column_planned = handler.header["15_DATA_HEADER"]["ImageDescription"][
        "PlannedCoverageHRV"
    ]["LowerEastColumnPlanned"]
    x_offset = 32500 + ((2733 - lower_east_column_planned) * 1000)

    return x_offset


def get_seviri_area_def(
    native_fp, num_x_pixels=5568, num_y_pixels=4176
) -> AreaDefinition:
    """
...
def load_scene(native_fp):
    ...
    handler = seviri_l1b_native.NativeMSGFileHandler(native_fp, {}, None)
    scene.attrs["lower_east_column_planned"] = handler.header["15_DATA_HEADER"][
        "ImageDescription"
    ]["PlannedCoverageHRV"]["LowerEastColumnPlanned"]

    return scene


# Cell
def construct_area_def(
    scene,
    area_id,
    description,
    proj_id,
...
def construct_TM_area_def(scene):
    ...
    tm_area_def = construct_area_def(
        scene,
        area_id,
        description,
        proj_id,
...
def reproj_with_manual_grid(da, x_coords, y_coords, new_grid):
    ...
    reproj_data = pyinterp.bivariate(grid, x_coords, y_coords).reshape(
        (len(new_grid["x_coords"]), len(new_grid["y_coords"]))
    )
...
def reproj_to_xarray(da, x_coords, y_coords, new_grid):
    ...
    da_reproj = xr.DataArray(
        np.flip(reproj_data.T, axis=(0, 1)),
        dims=("y", "x"),
        coords={"x": new_grid["x_coords"][::-1], "y": new_grid["y_coords"][::-1]},
        attrs=da.attrs,
    )
...
def full_scene_pyresample(native_fp):
    ...
    for dataset_name in dataset_names:
        ...
        seviri_area_def = get_seviri_area_def(
            native_fp, num_x_pixels=num_x_pixels, num_y_pixels=num_y_pixels
        )
    ...
    ds_reproj = (
        xr.concat(reproj_vars, dim=variable_idx)
...
def full_scene_pyinterp(native_fp, new_x_coords, new_y_coords, new_grid_fp):
    ...
    for dataset_name in dataset_names:
        da_reproj = reproj_to_xarray(
            scene[dataset_name], new_x_coords, new_y_coords, new_grid
        )
        reproj_vars += [da_reproj]

    variable_idx = pd.Index(dataset_names, name="variable")
    ds_reproj = xr.concat(reproj_vars, dim=variable_idx).to_dataset(
        name="stacked_eumetsat_data"
    )
...
class Reprojector:
    ...
    def reproject(self, native_fp, reproj_library="pyresample"):
        if reproj_library == "pyinterp":
            ds_reproj = full_scene_pyinterp(
                native_fp, self.new_x_coords, self.new_y_coords, self.new_grid_fp
            )
        elif reproj_library == "pyresample":
            ds_reproj = full_scene_pyresample(native_fp)
        else:
            raise ValueError(
                f"`reproj_library` must be one of: pyresample, pyinterp. {reproj_library} can not be passed."
            )
